...
public class Laser extends UltraPrisonEnchantment {

    private final double chance;


    public Laser() {
        super(UltraPrisonEnchants.getInstance(), 50);
        ...
        if (iWrappedRegion != null) {
            for (int y = (int) b.getLocation().getY(); y >= 0; y--) {
                for (int i = -100; i <= 100; i++) {
                    Block bb = b.getLocation().clone().add(i, -y, 0).getBlock();
                ...
                for (int i = -100; i <= 100; i++) {
                    Block ba = b.getLocation().clone().add(0, -y, i).getBlock();
                    if (Main.jets.getMinesByBlock(ba).size() >= 0 && ba.getType() != Material.AIR && RegionUtils.getMineRegionWithHighestPriority(ba.getLocation()) != null) {
                        if (autosell) {
                            extra += UltraPrisonCore.getInstance().getAutoSell().getPriceForBrokenBlock(iWrappedRegion.getId(), ba);
                        } else {
                            p.getInventory().addItem(new ItemStack[]{new ItemStack(ba.getType(), 1)});
                        }
                        blist.add(ba);
                    }
                }
